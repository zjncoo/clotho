...
export default function CatalogPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    try {
      setStatus('Rimozione sfondo in corso...');
      
      let transparentBlob: Blob;

      // Se la libreria CDN è pronta usa l'AI, altrimenti usa l'immagine originale
      // @ts-expect-error window.imgly is loaded via CDN script
      if (typeof window !== 'undefined' && window.imgly?.removeBackground) {
        // @ts-expect-error window.imgly is loaded via CDN script
        transparentBlob = await window.imgly.removeBackground(file);
      } else {
  ...
  return (
    <>
      <Script
        src="https://cdn.jsdelivr.net/npm/@imgly/background-removal@1.4.5/bundle/index.js"
        strategy="lazyOnload"
      />
